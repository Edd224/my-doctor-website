  ...
  return (
    ...
    >
      <div className="px-5 sm:px-10 pb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 items-baseline gap-12 p-5 sm:p-8 bg-white text-xl text-black rounded-10 space-y-2 sm:space-y-5">
          {paragraphs.map((paragraph, index) => (
            <motion.p
              key={index}
              className="border-b sm:border-none border-text pb-8"
              variants={containerVariants}
              custom={index + 1}
            >
              {paragraph.text}
